...
def test_reward_paid_event_fires(multi, accounts, base_token, reward_token, chain, alice, bob):
    tx = multi.getReward()
...
def test_recovered_fires(multi, alice, reward_token, issue):
    tx = multi.recoverERC20(reward_token, 10 ** 10, {"from": alice})
