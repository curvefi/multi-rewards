...
def test_distributor_cannot_modify_reward(multi_reward, accounts, reward_token, alice, bob):
    with brownie.reverts():
...
def test_reward_unmodifiable(multi_reward, accounts, reward_token, account_id, account_id2):
    with brownie.reverts():
        multi_reward.addReward(reward_token, accounts[account_id], 3600, {'from': accounts[account_id2]})
...
def test_owner_notify_reward_amount(multi_reward, accounts, reward_token, alice, bob, base_token, chain):
...
def test_rando_notify_reward_amount(multi_reward, accounts, reward_token, alice, bob, charlie, base_token, chain):
    reward_token.approve(multi_reward, 10**19)
